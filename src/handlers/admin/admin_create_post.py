...
from messages import message_text as texts
...
async def create_title(call: CallbackQuery, state: FSMContext):
    ...
    if not int(data.get('ready_to_start')):
        await call.message.answer(texts.CANCEL_TEXT, reply_markup=admin_menu)
        await state.finish()
        return
    else:
...
async def create_text(message: Message, state: FSMContext):
    ...
    await message.answer(texts.CREATE_TEXT)
...
@dp.callback_query_handler(text=['1', '0'],
                           state=NewsItemState.want_to_add_photo)
async def add_photo(call: CallbackQuery, state: FSMContext):
    ...
    if not int(new_item.want_to_add_photo):
        await call.message.answer('Добавим пост без фотка',
...
async def is_wanted_to_add_photo(message: Message, state: FSMContext):
    ...
    data = await state.get_data()
    updat_post(data.get('title'), photo_id, data.get('text'))
    await message.answer(
        texts.POST_CREATED, reply_markup=admin_menu
    )
    author = message.from_user
    logging.info(f'Создан новый пост {author.username or author.id}')
    await state.finish()
